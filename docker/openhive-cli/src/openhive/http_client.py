...
class OpenHiveHTTPClient:
    """HTTP client for interacting with the OpenHive backend API."""

    def __init__(
        self,
        config: OpenHiveConfig | None = None,
    ...
    def __enter__(self) -> "OpenHiveHTTPClient":
        return self
...
def get_client(config: OpenHiveConfig | None = None) -> OpenHiveHTTPClient:
    """Get a configured HTTP client."""
    return OpenHiveHTTPClient(config=config)
