"""
Identity management for OpenHive CLI.
...
def get_session_path() -> Path:
    ...
    return Path.cwd() / ".openhive" / "session"


def load_session() -> str | None:
    """Load session ID from project-local .openhive/session."""
...
def save_session(session_id: str) -> None:
    """Save session ID to project-local .openhive/session."""
    session_path = get_session_path()
...
def get_current_handle(config: "OpenHiveConfig") -> str | None:
...
def get_or_create_identity(config: "OpenHiveConfig", name: str | None = None) -> str:
...
def regenerate_identity(config: "OpenHiveConfig", name: str | None = None) -> str:
...
def get_or_create_machine_id(config: "OpenHiveConfig") -> str:
