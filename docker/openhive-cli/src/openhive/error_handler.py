...
from openhive.exceptions import APIError, OpenHiveError


def format_error(error: Exception, verbose: bool = False) -> str:
    """Format an exception into a user-friendly message."""
    if isinstance(error, OpenHiveError):
        return _format_openhive_error(error, verbose)
...
def _format_openhive_error(error: OpenHiveError, verbose: bool) -> str:
...
def _format_connect_error(error: httpx.ConnectError, verbose: bool) -> str:
    lines = [
        "Error: Failed to connect to the OpenHive API",
