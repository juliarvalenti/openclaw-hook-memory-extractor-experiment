"""Config commands for OpenHive CLI."""
...
def show(ctx: typer.Context) -> None:
    ...
    try:
        ...
        if json_output:
            ...
            typer.echo(f"  CFN URL:     {config.server.cfn_url}")
...
def set_config(
    ...
    Examples:
        openhive config set server.api_url http://myhost:8000
        openhive config set --env local
    """
    try:
        if env:
            ...
            config.server.cfn_url = env_vals["cfn_url"]
            config.save()
