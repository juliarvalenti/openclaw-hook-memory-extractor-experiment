"""
Daemon commands for OpenHive CLI.
...
_DAEMON_PID_FILE = Path.home() / ".openhive" / "daemon.pid"
...
def _daemon_loop(config: OpenHiveConfig, handle: str) -> None:
    ...
    while running:
        ...
        try:
            with OpenHiveHTTPClient(config=config, max_retries=1) as client:
                # Heartbeat
                if now - last_heartbeat >= _HEARTBEAT_INTERVAL:
                    try:
                        client.post(f"/agents/{handle}/checkin", json={"status": "online"})
                        last_heartbeat = now
                ...
                try:
                    response = client.get(f"/agents/{handle}/runs/pending")
                    runs = response.json().get("runs", [])
                    for run in runs:
                        logger.info(f"Pending run: {run.get('id')}")
                        # Future: dispatch run to CFN
                except Exception as e:
...
def start(
    ...
    """Start the OpenHive daemon in the background."""
    try:
        ...
        # Windows — no fork
        typer.echo("Background daemon not supported on Windows. Run 'openhive daemon run' instead.")
    except Exception as e:
...
def start_cmd(
    ...
    """Start the OpenHive daemon."""
    start(ctx, restart=restart)


@app.command("stop")
def stop(ctx: typer.Context) -> None:
    """Stop the OpenHive daemon."""
    try:
...
def daemon_status(ctx: typer.Context) -> None:
    ...
    if json_output:
        ...
        if running:
            ...
            typer.echo("  Start with: openhive daemon start")
