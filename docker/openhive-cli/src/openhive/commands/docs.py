"""
Documentation commands for OpenHive CLI.
...
def _get_docs_root() -> Path:
    ...
    try:
        with resources.as_file(resources.files("openhive").joinpath("docs")) as docs_path:
            return docs_path
...
def docs_main(
    ...
    """
    Built-in documentation for OpenHive CLI.

    Examples:
        openhive docs concepts rooms
        openhive docs --list
        openhive docs search "stream"
    """
    ...
    if section == "search":
        if not topic:
            typer.secho("Usage: openhive docs search QUERY", fg=typer.colors.RED)
            raise typer.Exit(1)
    ...
    if doc_path:
        ...
        if full:
            ...
            typer.secho(f"openhive docs > {section} > {topic}", fg=typer.colors.BRIGHT_BLACK)
            typer.echo("")
...
def _print_doc_list(docs_root: Path) -> None:
    ...
    for section, topic, title in docs:
        ...
        cmd = f"openhive docs {section} {topic}" if section else "openhive docs"
...
def _print_section_list(docs_root: Path, section: str) -> None:
    ...
    for _, topic, title in docs:
        cmd = f"openhive docs {section} {topic}"
        typer.echo(f"  {cmd:<40} {title}")
